...
void main()
{
  ...
  const vec3 viewSpaceNormal =
    normalize((transpose(uniformParams.invView) * vec4(normal, 0.0)).xyz);
  ...
  DirectionalLight currentLight = directionalLightsBuffer[0];
  ...
  vec3 pointToLight = -currentLight.direction;

  vec3 fromPosToCamera = normalize(uniformParams.cameraWorldPosition - worldSpacePosition.xyz);
  vec3 fromPosToLight = normalize(pointToLight);                            
  vec3 surfaceNormal = normalize(normal);                                   
  vec3 halfVector = normalize(fromPosToLight + fromPosToCamera);            

  float NdotL = clampedDot(surfaceNormal, fromPosToLight);
  float NdotH = clampedDot(surfaceNormal, halfVector);
  float NdotV = clampedDot(surfaceNormal, fromPosToCamera) + 0.00001;

  vec3 reflectDir = reflect(-pointToLight, surfaceNormal);

  float curvature = length(fwidth(surfaceNormal));
  float edgeFactor = pow(curvature, 0.5) * 1.0;

  vec3 diffuse = currentLight.color * currentLight.intensity * NdotL;

  float specPower = mix(32, 256, edgeFactor);
  float specIntensity = pow(NdotH, specPower);

  vec3 specular = currentLight.color * specIntensity * (1.0 + edgeFactor * 2.0);
